...
const main = async () => {
  ...
  const pushChunk = () => {
    while (stream.getBufferSize() < BUFFER_SIZE_TARGET) {
      const buffer = new Float32Array(SAMPLE_RATE * CHANNELS * 0.1);
      ...
      const pushedFrames = stream.pushAudioChunk(undefined, buffer);
      console.log(`Pushed ${pushedFrames} frames, position delta: ${currentSample - stream.getPosition()}`);
    }
  }

  pushChunk();

  const interval = setInterval(pushChunk, 100);
